...
| 0:40 | "A swarm of simulated robots learns to navigate it. Every map version is a Weave evaluation on a fixed task set: v0 is the baseline." | `out/<run>/swarm.mp4` (first segment), Weave evaluations page |
...
| 1:55 | "The map outlives the robot generation: it exports to the Nav2 format a Go2 or G1 stack loads today." | `out/<run>/go2_on_map.mp4` (Genesis Go2 on the scanned map) |
