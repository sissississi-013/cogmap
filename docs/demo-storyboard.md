...
3. The Nav2 proof step at the end of `run_venue.sh` needs Docker: run `orb start` first (OrbStack; the `cogmap-nav2`
   image is already built and cached). Without it the step is skipped and everything else still works.
