...
| 1:55 | "The map outlives the robot generation: it exports to the Nav2 format a Go2 or G1 stack loads today — here is a stock ROS 2 map_server publishing it." | `out/<run>/go2_walk.mp4` (Genesis Go2 walking the planned path) + `docs/proof/nav2_map_server.log` |
