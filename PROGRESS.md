...
- 04:38 Rescan mode (real-world change): `--rescan <video_b>` registers a second walkthrough to map A (FFT xcorr over rotation/scale), diffs it, and applies confident differences to the simulated truth so the loop repairs a REAL change. Office A vs 90 s office B: registered at 354°, score 0.56, diff mostly reconstruction noise (as expected, nothing moved). Test running (out/rescan_test). Storyboard documents the optional second venue pass.
- 04:37 Rescan round validated (out/rescan_test): real diff between two office walkthroughs → 19 collisions → 2 repairs → 100%/0 collisions; scripted round 2 then pre-empted by patrol. README documents `--rescan`; tests/test_rescan.py (14 tests total).
- 04:43 Storyboard dry run with the literal morning commands (`./run_venue.sh footage/raw/venue.mov out/venue`) passed end to end in 4.5 min incl. Nav2 proof (97-pose path). Placeholder footage/raw/venue.mov + out/venue removed so the real recording is unambiguous. OrbStack stopped to free RAM (checklist says `orb start`).
- 04:49 Five-seed sweep (synthetic, rule+patrols+explore): after-repair success 98–99% ± 2–3% in every round; table in README (scripts/seed_sweep.sh, scripts/seed_summary.py).
